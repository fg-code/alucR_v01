...
#p_raster | RasterStack of suitabilities for the specific land use classes to model. Layer names "lc1", "lc2"... with the numbers refering to the land cover classes in the initial land cover map lc
#nochange.lc | (optional) vector of charachter naming the land cover classes which do not change during the modelling experiment example c("lc5","lc6") when class 5 refers for example to water and 5 and 6 refer to water. Nochange classes should not be included in the p_raster RasterStack (will be dropped if any)
...
alucR_prep1 <- function (lc, p_raster, nochange.lc, natural.lc ){ 
  ...
  lc_suit <- as.numeric(gsub("lc","",tolower(names(p_raster))))
